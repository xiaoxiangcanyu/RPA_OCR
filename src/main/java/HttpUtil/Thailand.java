 ...
 * 泰国工厂发票数据处理
 */
public class Thailand extends BaseUtil {
...
//        String filePath1 = args[4];
        ...
        String filePath = "C:\\Users\\songyu\\Desktop\\haier_rpa所有资料\\OCR_Data\\项目交接文档\\项目交接文档\\AP\\新加坡用horis的国家\\泰国工厂\\pic\\";
        String OCR_filepath = "C:\\Users\\songyu\\Desktop\\haier_rpa所有资料\\OCR_Data\\项目交接文档\\项目交接文档\\AP\\新加坡用horis的国家\\泰国工厂\\pic\\test\\OCR.xls";
        String SAP_filepath = "C:\\Users\\songyu\\Desktop\\haier_rpa所有资料\\OCR_Data\\项目交接文档\\项目交接文档\\AP\\新加坡用horis的国家\\泰国工厂\\pic\\test\\SAP.xls";
        String filePath1 = "C:\\Users\\songyu\\Desktop\\haier_rpa所有资料\\OCR_Data\\项目交接文档\\项目交接文档\\AP\\新加坡用horis的国家\\泰国工厂\\pic\\1.xls";
        //从路径中获取companycode和filename
        List<DataDO> SAPList = new ArrayList<>();
        List<DataDO> OCRList = new ArrayList<>();

        try {
            clearTLFunction(companyCode, filePath, OCR_filepath, SAP_filepath, filePath1,SAPList,OCRList);
        } catch (Exception e) {
...
//            templateNot(companyCode, fileName, OCR_filepath, msg);
     ...
     * @throws Exception
     */
    public static void clearTLFunction(String companyCode, String filePath, String OCR_filePath, String SAP_filPath, String filePath1,List<DataDO> SAPList,List<DataDO> OCRList) throws Exception {
        ...
        for (File file1:files){
            if (file1.getName().endsWith(".jpg")){
                try {
                    ...
                    String number = filePath.substring(filePath.lastIndexOf("-") + 1, filePath.indexOf(".jpg"));
                    System.out.println("输出单号：" + number);
                    ...
                    if (jsonObject.get("result").toString().contains("success")) {
                        ...
                        if (number.equals(InvoiceReferenceNumber)) {
                            ...
                            if (amountNum > 0 && amountNum == posNum && amountNum == priceNum && amountNum == quantityNum) {
                                ...
                                if (dataDOS_ASP.size() > 0) {
                                    ...
                                    SAPList.addAll(dataDOS_ASP);
                                    //生成SAP表
//                        excelOutput_SAP(dataDOS_ASP, SAP_filPath);
                                }
                                OCRList.addAll(dataDOS_OCR);
                                //生成OCR表
//                    excelOutput_OCR(dataDOS_OCR, OCR_filePath);
                            ...
                            dataDO.setOCRStatus("InvoiceReferenceNumber Different");
...
//                    templateNot(companyCode, fileName, OCR_filePath, msg);
        ...
        excelOutput_SAP(SAPList, SAP_filPath);
        excelOutput_OCR(OCRList, OCR_filePath);
